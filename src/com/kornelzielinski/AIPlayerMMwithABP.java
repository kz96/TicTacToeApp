...
public class AIPlayerMMwithABP extends AIPlayer {
    ...
    @Override
    int[] move() {
        int[] result = minimax(2, oppContent, Integer.MIN_VALUE, Integer.MAX_VALUE);
    ...
    private int[] minimax (int depth, Content player, int alpha, int beta) {
        ...
        if (nextMoves.isEmpty() || depth == 0) {
            ...
            for (int[] move : nextMoves) {
                ...
                if (player == myContent) {
                    score = minimax(depth - 1, oppContent, alpha, beta)[0];
                    if (score > alpha) {
                        alpha = score;
                        bestRow = move[0];
                        bestColumn = move[1];
                    ...
                    score = minimax(depth - 1, myContent, alpha, beta)[0];
                    if (score < beta) {
                        beta = score;
                        bestRow = move[0];
                        bestColumn = move[1];
